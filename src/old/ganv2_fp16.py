...
from tensorflow.keras.layers import LeakyReLU, UpSampling2D, Conv2D, Input
...
class DCGAN():
    ...
    def save_imgs(self, epoch, generatedImgs):
            # Rescale images 0 - 1
            r, c = 3, 3
            generatedImgs = np.float32(generatedImgs)
    ...
    def train(self, dir, pcaModel, epochs, batch_size=128, save_interval=50, currEpoch=0):
        ...
        augmentation = aug.flow_from_directory(dir, batch_size = batch_size, class_mode="sparse", target_size=(224,224))
        ...
        for x, y in augmentation:
            ...
            cv2.normalize(x, x, -1, 1, cv2.NORM_MINMAX)
            x = np.float16(x)
            Z_pcaFeature = pcaModel.transform(features.reshape(features.shape[0], -1))
            gen_imgs = self.generator.predict(np.float16(Z_pcaFeature))
            # Train the discriminator
            d_loss_real = self.discriminator.train_on_batch(x, valid)
            d_loss_fake = self.discriminator.train_on_batch(gen_imgs, fake)
            d_loss = 0.5 * np.add(d_loss_real, d_loss_fake)
            #d_loss = self.discriminator.train_on_batch(np.concatenate((x, gen_imgs)), np.concatenate((valid, fake)))

            #if self.epoch % 2 == 0:
                #g_loss = self.combined.train_on_batch(noise, valid)
            #else:
            g_loss = self.combined.train_on_batch(Z_pcaFeature, valid)
            
            
            
            # Plot the progress
            print ("%d [D loss: %f, acc.: %.2f%%] [G loss: %f, acc.: %.2f%%]" % (self.epoch, d_loss[0], 100*d_loss[1], g_loss[0], g_loss[1]))
